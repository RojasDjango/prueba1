...
namespace App\Livewire\Admin\Ejecutoras\Vercomando;
...
class VercomIndex extends Component
{   
    use WithPagination;
    ...
    public function render()
    {
        ...
        // Construir la consulta base filtrando por el usuario creador
        $query = Ejecutora::where('created_by', $userId)
            ->with('user')
            ->when($this->searchcomando, function($query) {
                $query->where('name', 'like', '%'.$this->searchcomando.'%');
            })
       ...
       // Ordenación
        ...
        return view('livewire.admin.ejecutoras.vercomando.vercom-index',compact('ejecutoras'));
    }
}
