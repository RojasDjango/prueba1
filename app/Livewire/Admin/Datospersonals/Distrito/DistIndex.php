...
namespace App\Livewire\Admin\Datospersonals\Distrito;
...
class DistIndex extends Component
{
    use WithPagination;
...
//****************************************distrito
    ...
    if ($userData && $userData->departamento_id) {
        $distrito = Distrito::find($userData->distrito_id);
        if ($distrito) {
            $this->searchdistrito = $distrito->name;
        }
    }
    //fin ***************************
    ...
    $datopersonals = $query->paginate($this->perPage);
        ...
        return view('livewire.admin.datospersonals.distrito.dist-index',compact('datopersonals'));
    }
}
